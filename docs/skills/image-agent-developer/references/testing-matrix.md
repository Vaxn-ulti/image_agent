...
- Runtime probe checks host FSL commands under `/home/yyf/project/MCI_project/tools/fsl`, GPU visibility with `nvidia-smi`, and MRtrix command availability inside the fixed toolbox image `pennlinc/qsiprep:1.0.2`.
...
- Detection uses `eddy_cuda*` glob (accepts `eddy_cuda11.0`, `eddy_cuda10.2`, etc.); passes when the locked QSIPrep image, currently `pennlinc/qsiprep:1.0.2`, exposes `eddy_cuda11.0` at `/app/.pixi/envs/qsiprep/bin/`.
