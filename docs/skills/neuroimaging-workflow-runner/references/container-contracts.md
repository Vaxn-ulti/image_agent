...
Production `dwi_fast_gpu_dti` does not use that image as a full QSIPrep workflow. It uses host FSL at `<REMOTE_HOME>/project/MCI_project/tools/fsl` for GPU `eddy_cuda` and FSL registration commands, and uses `pennlinc/qsiprep:26.0.0` only to access MRtrix commands.
...
```text
python -m app.workflows.dwi_fast_dti run --bids {bids} --out {output} --work {work} --resources {resources} --fsl-dir <REMOTE_HOME>/project/MCI_project/tools/fsl --mrtrix-image pennlinc/qsiprep:26.0.0 --max-runtime-sec 2100 --require-gpu-eddy
```
