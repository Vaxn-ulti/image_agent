...
| --- | --- | --- | --- | --- |
| `t1_deepprep` | `production_supported` | Launch only for T1-compatible series after backend preflight and confirmation. | Real completed task, parsed DeepPrep/FreeSurfer stats, registered maps/tables/reports, and `/tasks/{task_id}/result-summary`. Current evidence includes real task ids 40 and 41. | DeepPrep and FreeSurfer official docs ground expected outputs. Container-native QC source ids may include `docs/rag/vendor/deepprep_official_container_usage.md` and `docs/rag/vendor/freesurfer_official_container_reconall.md`. |
...
| `bold_fmriprep_xcpd_report` | `incubation_reference` | Conditional remote-wrapper contract for fMRIPrep/XCP-D-style reports. The wrapper must use task-scoped env paths, redact logs, avoid fixed evidence-project paths, and treat XCP-D input as fMRIPrep-compatible derivatives rather than raw BIDS. | Promotion requires real remote task evidence that wrapper scripts honor `IMAGE_AGENT_TASK_*` paths, register fMRIPrep/XCP-D native reports, expose artifact manifests, and pass strict smoke without leaked paths. | docs/rag/workflows/bold_fmriprep_xcpd_report.md plus fMRIPrep/XCP-D vendor docs ground expected native reports. Do not call it production-ready unless wrapper and real task evidence exist. |
