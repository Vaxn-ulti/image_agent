...
- strict smoke acceptance JSON reports `project_contract_status=passed`, `upload_inventory_contract_status=passed`, `task_artifact_manifest_status=passed`, and `task_result_summary_status=passed` when `--project-id`, `--upload-session-id`, and `--task-id` are supplied.
