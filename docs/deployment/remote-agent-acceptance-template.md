...
- `smoke_gate.expected_model_wire_api=chat_completions` and `model_status.wire_api=chat_completions`
- `smoke_gate.expected_model_provider_profile=deepseek` and `model_status.provider_profile=deepseek`
- `smoke_gate.require_model_tool_loop` is absent and `model_status.capabilities.model_tool_loop=false`
...
- strict smoke acceptance JSON reports `smoke_gate.expected_model_wire_api=chat_completions`, `model_status.wire_api=chat_completions`, `smoke_gate.expected_model_provider_profile=deepseek`, `model_status.provider_profile=deepseek`, no `smoke_gate.require_model_tool_loop`, and `model_status.capabilities.model_tool_loop=false`;
