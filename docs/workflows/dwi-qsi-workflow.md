...
- `niter: 3` is the speed-oriented first-pass default. Override with `IMAGE_AGENT_DWI_QSIPREP_EDDY_NITER=5` for default eddy convergence.
...
## CUDA Eddy Single-Thread Behavior
...
Workflow containers are labeled with:
...
2. Check `docker ps --filter "label=image_agent.app=image_agent"` for labeled containers.
