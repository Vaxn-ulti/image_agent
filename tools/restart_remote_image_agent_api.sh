...
ROOT="${IMAGE_AGENT_ROOT:-<IMAGE_AGENT_ROOT>}"
...
PY
  ...
  if [[ "$active" != "[]" ]]; then
    printf 'active_task_drain:remediation run scoped dry-run before override: cd %s && PYTHONPATH=. %s scripts/reconcile_stale_tasks.py --max-age-hours 24 --check-containers --ta<REDACTED_API_KEY> <task_id>\n' "$API_DIR" "$PYTHON_BIN" >&2
