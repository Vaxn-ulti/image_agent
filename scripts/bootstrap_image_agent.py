...
def _is_external_model_gateway_model_target(provider: str, base_url: str) -> bool:
    provider_name = (provider or "").strip().lower()
    host = (urlsplit(base_url).hostname or "").lower()
    return provider_name == "external_model_gateway" or host == "<model-host>" or host.endswith(".<model-host>")
...
def _model_env_updates(
    *,
    ...
    if _is_external_model_gateway_model_target(values["provider"], values["base_url"]) and model_trust_env_proxy:
        raise SystemExit("external_model_gateway model gateway must use direct transport, not environment proxy")
