...
PROVIDER_PROFILES: dict[str, dict[str, Any]] = {
    ...
    "external_model_gateway": {
        "base_url": "<model-base-url>",
        "model": "gpt-5.5",
        "wire_api": "responses",
        "api_key_env": ["external_model_gateway_API_KEY", "OPENAI_API_KEY"],
        "base_url_env": ["external_model_gateway_BASE_URL", "OPENAI_BASE_URL"],
        "model_env": ["external_model_gateway_MODEL", "OPENAI_MODEL", "MODEL"],
        "review_model_env": ["external_model_gateway_REVIEW_MODEL", "OPENAI_REVIEW_MODEL", "REVIEW_MODEL"],
        "wire_api_env": ["external_model_gateway_WIRE_API", "OPENAI_WIRE_API"],
    },
    "krill": {
...
def _provider_profile(provider: str) -> str:
    ...
    aliases = {
        "open_ai": "openai",
        "openai": "openai",
        "raw_chat": "external_model_gateway",
        "external_model_gateway": "external_model_gateway",
        "krill": "krill",
        "deepseek": "deepseek",
        "deep_seek": "deepseek",
        "glm": "glm",
...
def _is_external_model_gateway_direct_target(profile_name: str, base_url: str) -> bool:
    if profile_name == "external_model_gateway":
        return True
    host = (urlsplit(base_url).hostname or "").lower()
    return host == "<model-host>" or host.endswith(".<model-host>")
...
class ModelConfig:
    ...
    def from_env(cls) -> "ModelConfig":
        ...
        if _is_external_model_gateway_direct_target(profile_name, base_url):
