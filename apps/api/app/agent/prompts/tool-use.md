...
Use `read_project_context`, `list_workflows`, `list_data_candidates`, `select_incubation_dataset`, `preflight_workflow`, `retrieve_reference_context`, `create_workflow_task`, `read_task`, `read_task_events`, `read_result_summary`, and `observe_repair_task` for fixed workflow operation. Candidate selection may prepare a confirmation but must keep `production_task_created=false`.

`observe_repair_task` is read-only. It may inspect task status, safe task events, remote-log summaries, and result-summary state, then draft repair suggestions. It must not retry, rerun, or create tasks; any retry or rerun must go through registry preflight and human confirmation first.
