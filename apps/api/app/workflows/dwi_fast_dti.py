...
IMAGE = MRTRIX_IMAGE
FSL_DIR = Path(os.environ.get("IMAGE_AGENT_FSL_DIR", "<REMOTE_HOME>/project/MCI_project/tools/fsl"))
...
DEFAULT_TEMPLATE_CANDIDATES = [
    "<REMOTE_HOME>/project/MCI_project/tools/fsl/data/standard/MNI152_T1_2mm.nii.gz",
    "<REMOTE_HOME>/Project/qsitest_20260507/templateflow/tpl-MNI152NLin2009cAsym/tpl-MNI152NLin2009cAsym_res-02_T1w.nii.gz",
    "<REMOTE_HOME>/Project/qsitest_20260507/templateflow/tpl-MNI152NLin6Asym/tpl-MNI152NLin6Asym_res-02_T1w.nii.gz",
]
DEFAULT_ATLAS_CANDIDATES = [
    (
        "MNI152_HarvardOxford_cort_maxprob_thr25_2mm",
        "<REMOTE_HOME>/project/MCI_project/tools/fsl/data/atlases/HarvardOxford/HarvardOxford-cort-maxprob-thr25-2mm.nii.gz",
    ),
    (
        "MNI152_HarvardOxford_cort_maxprob_thr25_2mm",
        "<REMOTE_HOME>/nilearn_data/fsl/data/atlases/HarvardOxford/HarvardOxford-cort-maxprob-thr25-2mm.nii.gz",
    ),
    (
        "MNI152_Schaefer2018_200Parcels_7Networks",
        "<REMOTE_HOME>/nilearn_data/schaefer_2018/Schaefer2018_200Parcels_7Networks_order_FSLMNI152_2mm.nii.gz",
    ),
    (
        "MNI152_Juelich_maxprob_thr25_2mm",
        "<REMOTE_HOME>/nilearn_data/fsl/data/atlases/Juelich/Juelich-maxprob-thr25-2mm.nii.gz",
    ),
]
    ...
    if not paths["atlas"].exists():
        ...
        if atlas_source is not None:
            ...
            raise RuntimeError(
                ...
                "or install a cached Schaefer/Juelich/HarvardOxford atlas under <REMOTE_HOME>/nilearn_data."
            )
...
def run_fast_dti(
    ...
    *,
    ...
    (qc_dir / "dwi_fast_gpu_dti_provenance.json").write_text(
        json.dumps(
            {
                "basis_script": "<REMOTE_HOME>/project/MCI_project/scripts/run_fast_gpu_dti_features.sh",
...
def _main(argv: list[str] | None = None) -> int:
    ...
    run_parser.add_argument("--ta<REDACTED_API_KEY>", type=int, default=0)
...
def write_validate_outputs(out_dir: Path, task_id: int, workflow_type: str) -> Path:
    ...
    provenance.write_text(
        json.dumps(
            {
                "image": IMAGE,
                "script_source": "<REMOTE_HOME>/project/MCI_project/scripts/run_fast_gpu_dti_features.sh",
                "atlas": DEFAULT_ATLAS,
