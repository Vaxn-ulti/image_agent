...
describe('AgentPage', () => {
  ...
  it('keeps workflow quick actions available after the chat has messages', async () => {
    vi.mocked(api.listSeries).mockResolvedValueOnce([
      {
        confidence: 0.99,
        format: 'NIFTI',
    ...
    vi.mocked(api.runAgent)
      .mockResolvedValueOnce({
        agent_run_id: 'agent_run_overview_123',
        answer: '当前项目有 1 个 T1 序列，可以准备 T1 处理确认。',
        response_source: 'backend_context',
        status: 'answered',
      })
      .mockResolvedValueOnce({
    ...
    await userEvent.type(screen.getByLabelText('Agent query'), '先分析一下当前数据');
    await userEvent.click(screen.getByRole('button', { name: 'Send' }));
    expect(await screen.findByText('当前项目有 1 个 T1 序列，可以准备 T1 处理确认。')).toBeInTheDocument();

    await userEvent.click(await screen.findByRole('button', { name: 'Prepare T1 DeepPrep confirmation for series 11' }));

    expect(api.runAgent).toHaveBeenLastCalledWith(
      13,
      'Prepare workflow confirmation for t1_deepprep_anat_report on series 11. Do not create or launch a task yet.',
    );
    expect(await screen.findByText('Approval required')).toBeInTheDocument();
